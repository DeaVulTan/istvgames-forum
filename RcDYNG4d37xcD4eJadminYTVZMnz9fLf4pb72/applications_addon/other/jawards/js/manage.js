...
{	
	...
	{
		titleSpan.innerHTML = "Сохранение...";
...
function putFailed()
{
	...
	{
		titleSpan.innerHTML = "<span style='color:red;'>Неудачно</span>";
		hasFailed           = true;
...
function changeStatus_category(id, numAwards)
{
	...
	catEnablerImg.innerHTML = "<img src='" + stylesURL + "/images/loading.gif' title='Загрузка...' />";
...
function changeStatus_awards(id, newStatus)
{
	...
	{
		alert("Вы не можете изменить видимость этой награды, потому что она уже будет скрыта, поскольку категория отключена.");
		
		return;
	}
	
	var awardsEnablerImg       = document.getElementById("awards_enabler_" + id);
	awardsEnablerImg.innerHTML = "<img src='" + stylesURL + "/images/loading.gif' title='Загрузка...' />";
...
function changeShow(id)
{
	...
	ShowImg.innerHTML = "<img src='" + stylesURL + "/images/loading.gif' title='Загрузка...' />";
...
{
	var name = prompt("Введите название для категории", "");
...
function addCategory_continue( name )
{
	new Ajax.Request(ipb.vars['base_url'] . replace("&amp;", "&") + 'app=jawards&module=ajax&section=manage&do=addCategory&secure_key=' + ipb.vars['md5_hash'],
	{
		...
		onSuccess: function(t)
		{
			...
			{
				prompt("Добавление этой категории не удалось");
				return;
			}
		},
	});
