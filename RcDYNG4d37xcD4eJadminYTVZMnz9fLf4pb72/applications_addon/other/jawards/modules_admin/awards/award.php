...
class admin_jawards_awards_award extends ipsCommand
{
	...
	function main()
	{
		...
		<div class='section_title'>
		  <h2>Управление наградами пользователей</h2>
	    </div>";
		...
		<div class='acp-box'>
 		  <h3>Искать пользователя</h3>
		  <div class='tablerow1' style='padding:10px;' align='center'>
		  	Введите имя пользователя ниже, чтобы управлять его наградами. Оставьте пустым, чтобы получить всех пользователей.<br />
			<br />
			<input type='text' name='query' size='35' />
		  </div>
		  <div class='acp-actionbar' align='center'>
		    <input type='submit' value=' Поиск ' class='realbutton' />
		  </div>
		</div>
		</form>";
	}
	...
	function search()
	{
		...
		if(!$num)
		{
			...
			<div class='information-box' style='margin-bottom:15px;'>
			  <h4><img src={$this->settings['skin_acp_url']}/images/icons/bullet_error.png' /> Нет результатов!</h4>
		...
		$this->registry->getClass('output')->html_main .= "
		<div class='acp-box'>
 		  <h3>Результаты поиска: {$this->request['query']}</h3>
		  {$rows}
		</div>";
	}
	...
	function user()
	{
		...
		if($field)
		{
				...
				<div class='acp-box'>
				  <h3>Выдача награды {$editingMember['members_display_name']}</h3>
				  <table with='100%' class='double_pad'>
					<tr>
					  <td class='tablerow1' width='35%'><strong>Выбор награды</strong></td>
					  <td class='tablerow2'>{$field}</td>
					</tr>
					<tr>
					  <td class='tablerow1'>
						<strong>Примечание</strong><br />
						<div class='desctext'>Обычное примечание к награде</div>
					  </td>
					  <td class='tablerow2'>
				  ...
				  <div class='acp-actionbar'>
					  <input type='submit' value=' Выдать награду ' class='realbutton' />
					</div>
				</div>
				</form>";
		}
		...
		if(!$currentRows)
		{
			$currentRows = "
			<tr>
			  <td colspan='4' align='center'>{$editingMember['members_display_name']} не имеет \"текуших\" наград</td>
			</tr>";
		}
		...
		if(!$removedRows)
		{
			$removedRows = "
			<tr>
			  <td colspan='4' align='center'>{$editingMember['members_display_name']} не имеет \"удалённых\" наград</td>
			</tr>";
		}
		...
		<div class='acp-box'>
		  <h3>Награды {$editingMember['members_display_name']}</h3>
          <div id='tabstrip_usersAwards' class='ipsTabBar with_left with_right'>
			<ul id='tab_awards' class='tab_bar no_title'>
	          <li id='tab_current' class=''>Текущие награды</li>
		      <li id='tab_removed' class=''>Удалённые награды</li>
		    </ul>
		  </div>
		  ...
		  <div id='tab_current_content'>
		    <table with='100%' class='ipsTable double_pad'>
		      <tr>
				<th width='15%'><div align='center'>Изображение награды</div></th>
				<th width='25%'>Значок награды</th>
	...
	function delete()
	{
		...
		# Redirect
		$this->registry->output->global_message = "Награда удалена";
		$this->registry->output->silentRedirectWithMessage($url);
	}
	...
	function give()
	{
		...
		$this->registry->output->global_message = "Награда выдана";
