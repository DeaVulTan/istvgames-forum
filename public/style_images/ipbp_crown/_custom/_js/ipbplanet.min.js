...
(function (aM, C) {
    ...
    a.event = {
        add: function (a1, a5, ba, a3) {
            ...
            var bb = a6.events = a6.events || {}, a4 = a6.handle,
                a4;
            if (!a4) {
        ...
        special: {
            ...
            live: {
                add: function (aY) {
                    a.event.add(this, aY.origType, a.extend({}, aY, {
                        handler: V
                    }))
                },
                remove: function (aZ) {
    ...
    return this.each(function () {
        ...
        jQuery(window).scroll(function () {
            if (!jQuery.support.hrefNormalized) {
                b.css({
                    position: "absolute",
                    top: jQuery(window).scrollTop() + jQuery(window).height() - a.ieOffset
                })
            }
            if (jQuery(window).scrollTop() >= a.min) {
...
(function (a) {
    IPBPlanet = (function () {
        return {
            ...
            boundDropdowns: {},
            init: function (b) {
                IPBPlanet.storage_ns = b.storage_ns;
                if (a.cookie("ipbplanet_storage_" + b.storage_ns) != null) {
                    IPBPlanet.storage = a.parseJSON(a.cookie("ipbplanet_storage_" + b.storage_ns))
                }
                if (IPBPlanet.storage.shides == undefined) {
                    IPBPlanet.storage.shides = {}
                }
                if (IPBPlanet.storage.backgrounds == undefined) {
                    IPBPlanet.storage.backgrounds = {}
                }
                IPBPlanet.$tmp = a("<div />");
                a("table").attr("cellspacing", "0");
                a("body").append('<div id="ipbp-elements-storage" />');
                IPBPlanet.$storage.wrapper = a("#ipbp-elements-storage")
            },
            store: function (b) {
                a.extend(true, IPBPlanet.storage, b);
                IPBPlanet.commit()
            },
            commit: function () {
                a.cookie("ipbplanet_storage_" + IPBPlanet.storage_ns, JSON.stringify(IPBPlanet.storage), {
                    expires: 10,
                    path: "/"
                })
            },
            $store: function (b, d, c) {
                if (c == undefined) {
                    c = false
                }
                IPBPlanet.$storage[d] = a(b);
                var e = false;
                if (IPBPlanet.$storage[d].parent() != IPBPlanet.$storage.wrapper && c) {
                    if (IPBPlanet.$storage[d].parent().hasClass("ipbp-storable")) {
                        e = IPBPlanet.$storage[d].parent()
                    }
                    IPBPlanet.$storage[d].appendTo(IPBPlanet.$storage.wrapper);
                    if (e !== false && e.children().length == 0) {
                        e.remove()
                    }
                }
                return IPBPlanet.$storage[d]
            },
            $switch: function (b, g, e, c) {
                if (c == undefined) {
                    c = "inner"
                }
                var d;
                if (IPBPlanet.$storage[b] != undefined) {
                    d = IPBPlanet.$storage[b]
                } else {
                    if (IPBPlanet.$storage["_switch_" + g + "_to_" + e] == undefined) {
                        d = IPBPlanet.$store(b, "_switch_" + g + "_to_" + e)
                    } else {
                        d = IPBPlanet.$storage["_switch_" + g + "_to_" + e]
                    }
                }
                var f;
                if (c == "inner") {
                ...
                f(d, IPBPlanet.$storage[g], IPBPlanet.$storage[e])
            },
            ready: function (c) {
                var b = function () {
                    if (a.isArray(c)) {
                ...
                if (IPBPlanet.requiredFeatures === false || IPBPlanet.requiredFeatures.length !== 0) {
                    a(window).bind("ipbplanet_setup_complete", function (e, d) {
                        d();
                        b()
                    })
                } else {
            ...
            disableSetup: function () {
                IPBPlanet.requiredFeatures = []
            },
            requireSetupFor: function (b) {
                IPBPlanet.requiredFeatures = b
            },
            setup: function (b, c) {
                IPBPlanet.requiredFeatures = a.grep(IPBPlanet.requiredFeatures, function (d, e) {
                    return d != b
                });
                IPBPlanet.setupStack.push(c);
                if (IPBPlanet.requiredFeatures.length === 0) {
                    a(window).trigger("ipbplanet_setup_complete", function () {
                        IPBPlanet.setupStack.each(function (e, d) {
                            d()
                        })
                    })
                }
            ...
            topLink: function (b) {
                if (b == undefined) {
                    b = {
                        element: "#ipbp-toplink"
                    }
                }
                a(b.element).topLink({
                    min: b.min || 200,
                    fadeSpeed: b.fadeSpeed || 500
                });
                a(b.element).click(function (c) {
                    c.preventDefault();
                    a.scrollTo(0, b.scrollSpeed || 300)
                })
            },
            shide: function (c) {
                if (!c.duration) {
                ...
                a(c.element).wrapAll('<div id="ipbp-anim-wrapper-' + c.ns + '" />').wrapAll('<div id="ipbp-anim-' + c.ns + '" />');
                var d = a("#ipbp-anim-wrapper-" + c.ns);
                var b = a("#ipbp-anim-" + c.ns);
                c.$animation = b;
                c.$animationWrapper = d;
                d.css({
                    overflow: "hidden"
                });
                    ...
                    c.state = {
                        _after: function (e) {
                            if (e.callback != undefined) {
                                e.callback(a, e, IPBPlanet.storage.shides[e.ns])
                            }
                            if (e.before != undefined) {
                                e.before(a, e, IPBPlanet.storage.shides[e.ns])
                            }
                            if (e.after != undefined) {
                                e.after(a, e, IPBPlanet.storage.shides[e.ns])
                            }
                            if (e.complete != undefined) {
                                e.complete(a, e, IPBPlanet.storage.shides[e.ns])
                            }
                        }
                    };
                    if (c.direction == "top") {
                        c.state.shown = function (f, e) {
                            e.state._after(e)
                        };
                        c.state.hidden = function (f, e) {
                            e.$animation.css({
                                marginTop: -e.$animation.outerHeight(),
                                opacity: 0
                            });
                            e.state._after(e)
                        }
                        ...
                        c.state.shown = function (f, e) {
                            IPBPlanet.superShideShow(e);
                            e.state._after(e)
                        };
                        c.state.hidden = function (f, e) {
                            IPBPlanet.superShideHide(e);
                            e.state._after(e)
                        }
                    }
                }
                if (IPBPlanet.storage.shides[c.ns] != undefined) {
                    if (IPBPlanet.storage.shides[c.ns]) {
                        c.state.shown(a, c)
                    } else {
                        c.state.hidden(a, c)
                    }
                } else {
                    if (c.showByDefault != undefined || c.showByDefault == false) {
                        IPBPlanet.storage.shides[c.ns] = false;
                        c.state.hidden(a, c)
                    } else {
                        IPBPlanet.storage.shides[c.ns] = true;
                        c.state.shown(a, c)
                    }
                }
                a(c.trigger).click(function () {
                    if (c.callback != undefined) {
                        c.callback(a, c, !IPBPlanet.storage.shides[c.ns])
                    }
                    if (c.before != undefined) {
                        c.before(a, c, !IPBPlanet.storage.shides[c.ns])
                    }
                    if (!IPBPlanet.storage.shides[c.ns]) {
                        IPBPlanet.storage.shides[c.ns] = true;
                        a(c.trigger).switchClass("ipbp-shide-element-off", "ipbp-shide-element-on", 500)
                    } else {
                        IPBPlanet.storage.shides[c.ns] = false;
                        a(c.trigger).switchClass("ipbp-shide-element-on", "ipbp-shide-element-off", 500)
                    }
                    IPBPlanet.commit();
                    if (!IPBPlanet.storage.shides[c.ns]) {
                        IPBPlanet.superShideHide(c)
                    } else {
                        IPBPlanet.superShideShow(c)
                    }
                })
            },
            superShideShow: function (b) {
                if (b.direction == "top") {
                    b.$animation.animate({
                        marginTop: 0,
                        opacity: 1
                        ...
                        complete: function () {
                            if (b.complete != undefined) {
                                b.complete(a, b, IPBPlanet.storage.shides[b.ns])
                            }
                        }
                    })
                } else {
                    ...
                    b.after(a, b, IPBPlanet.storage.shides[b.ns])
                }
            },
            superShideHide: function (b) {
                if (b.direction == "top") {
                    b.$animation.animate({
                        marginTop: -b.$animation.outerHeight(),
                        opacity: 0
                    }, {
                        duration: b.duration,
                        queue: true,
            ...
            shides: function (b) {
                b.triggers.each(function (d, c) {
                    IPBPlanet.shide({
                        trigger: c,
                        element: b.elements[d]
                    })
                })
            },
            dropdowns: function (c) {
                var b = a(c.parent).find(c.only);
                b.each(function (e, f) {
                    var d = a(f);
                    d.addClass("ipbp-dropdown");
                    var g = d.children(":eq(1)");
                    g.hide();
                    g.bind("ipbp:dropdown:open", function (h) {
                        a(this).stop(true, true);
                        a(this).slideDown({
                            duration: 250,
                            queue: true
                        })
                    }).bind("ipbp:dropdown:close", function (h) {
                        a(this).delay(100);
                        a(this).slideUp({
                            duration: 250,
                            queue: true
                    ...
                    d.hover(function () {
                        g.trigger("ipbp:dropdown:open")
                    }, function () {
                        g.trigger("ipbp:dropdown:close")
                    })
                })
            },
            dropdown: function (c) {
                var b = a(c.element);
                var d = b.children(":eq(1)");
                d.hide();
                b.addClass("ipbp-dropdown");
                b.addClass("ipbp-dropdown-off");
                b.hover(function () {
                    IPBPlanet.dropdownShow(b, d, c)
                }, function () {
                    IPBPlanet.dropdownHide(b, d, c)
                })
            },
            dropdownShow: function (c, e, d) {
                var b = e;
                if (b.length > 0) {
                    b.stop(true, true);
                    b.slideDown({
                        duration: 250,
                        complete: function () {
                            b.addClass("ipbp-dropdown-on")
                        }
                    })
                } else {}
            },
            dropdownHide: function (c, e, d) {
                var b = e;
                if (b.length > 0) {
                    b.slideUp({
                        duration: 250,
            ...
            backgroundSwitcher: function (d) {
                if (d.element == undefined) {
                    d.element = "body"
                }
                if (IPBPlanet.storage.backgrounds[d.ns] != undefined) {
                    var b = IPBPlanet.storage.backgrounds[d.ns];
                    var e = a("#" + b);
                    if (e.length == 1) {
                        IPBPlanet.switchBackground({
                            id: b,
                            ns: d.ns,
                            href: e.attr("href"),
                            element: d.element,
                            switcher: function (g, f) {
                                f.defaultSwitcher()
                            }
                        }, d)
                    } else {
                        IPBPlanet.storage.backgrounds[d.ns] = undefined;
                        IPBPlanet.commit()
                    }
                }
                if (d.switcher == undefined) {
                    d.switcher = function (g, f) {
                        f.defaultSwitcher()
                    }
                }
                var c = function () {
                    a(d.triggers).children("a").each(function (g, f) {
                        a(f).click(function (h) {
                            h.preventDefault();
                            h.stopPropagation();
                            IPBPlanet.switchBackground({
                                id: a(f).attr("id"),
                ...
                if (d.delayApplyTriggers == undefined || d.delayApplyTriggers == false) {
                    c()
                }
                return c
            },
            switchBackground: function (c, b) {
                IPBPlanet.storage.backgrounds[c.ns] = c.id;
                IPBPlanet.commit();
                if (IPBPlanet.canOverwriteBackground()) {
                    b.defaultSwitcher = function () {
                        a(c.element).css({
                            "background-image": "url(" + c.href + ")"
                        })
                    };
                    c.switcher(a, b)
                }
            ...
            box: function (c) {
                ...
                if (c.element) {
                    a(c.element).hide();
                    c.content = a(c.element).html();
                    a(c.element).remove()
                }
                var b = new ipb.Popup(c.name, {
                    type: "pane",
                    modal: c.modal || false,
                    w: c.width || "500px",
                    h: c.height || "500px",
            ...
            balloon: function (g) {
                ...
                if (g.height) {
                    g.height = g.height + "px"
                }
                if (g.once == undefined) {
                    g.once = true
                }
                if (g.reference == undefined) {
                    g.reference = "absolute"
                }
                ...
                var b = a(g.trigger);
                var c = Element.extend(b[0]);
                var f;
                var e = function () {
                    f = new ipb.Popup(g.name, {
                        type: "balloon",
                        stem: true,
                        modal: g.modal || false,
                        w: g.width || "500px",
                        ...
                        attach: {
                            target: c,
                            position: g.position || "auto"
                        }
                    });
                    if (g.reference == "fixed") {
                        a("#" + g.name + "_popup").css({
                            position: "fixed"
                        })
                    }
                    if (g.afterCreation != undefined) {
                        g.afterCreation(a)
                    }
                };
                e();
            ...
            creditsBox: function () {
                IPBPlanet.box({
                    name: "credits-box",
                    trigger: ".trigger-credits-box",
                    element: "#ipbp-credits-box"
                })
            },
            canOverwriteBackground: function () {
                return ipb == undefined || ipb.profile == undefined || ipb.profile.customization == undefined || ipb.profile.customization != 1
            }
        }
    })()
})(jQuery);